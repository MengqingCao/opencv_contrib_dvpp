...
void cvtColorTestDVPP(int code, int cn, int dcn = 3, float diff = 0.0f)
{
    cv::cann::setDevice(DEVICE_ID);
    Mat cpuRet, npuRet;
    ...
    cv::cvtColor(img8U, cpuRet, code, dcn);
    cv::cann::cvtColordvpp(img8U, npuRet, code, dcn);
    EXPECT_MAT_NEAR(cpuRet, npuRet, diff);

    // cv::cvtColor(img16U, cpuRet, code, dcn);
    // cv::cann::cvtColordvpp(img16U, npuRet, code, dcn);
    // EXPECT_MAT_NEAR(cpuRet, npuRet, diff);

    // cv::cvtColor(img32F, cpuRet, code, dcn);
    // cv::cann::cvtColordvpp(img32F, npuRet, code, dcn);
    // EXPECT_MAT_NEAR(cpuRet, npuRet, diff);
    cv::cann::resetDevice();
}
...
TEST(CVT_COLOR_DVPP, BGRA2BGR) { cvtColorTestDVPP(COLOR_BGRA2BGR, 4); }

TEST(CVT_COLOR, BGR2RGBA) { cvtColorTest(COLOR_BGR2RGBA, 3, 4); }
// // the last channel of RGBA and BGRA in dvpp is 0, while which is 255 in opencv
// TEST(CVT_COLOR_DVPP, BGR2RGBA) { cvtColorTestDVPP(COLOR_BGR2RGBA, 3, 4); }
// TEST(CVT_COLOR_DVPP, BGR2BGRA) { cvtColorTestDVPP(COLOR_BGR2BGRA, 3, 4); }

TEST(CVT_COLOR, RGBA2BGR) { cvtColorTest(COLOR_RGBA2BGR, 4); }
TEST(CVT_COLOR, BGR2RGB) { cvtColorTest(COLOR_BGR2RGB, 3); }
TEST(CVT_COLOR_DVPP, BGR2RGB) { cvtColorTestDVPP(COLOR_BGR2RGB, 3); }
...
TEST(CVT_COLOR_DVPP, BGR2GRAY) { cvtColorTestDVPP(COLOR_BGR2GRAY, 3, 1, 10.0f); }

TEST(CVT_COLOR, RGB2GRAY) { cvtColorTest(COLOR_RGB2GRAY, 3, 1, 10.0f); }
TEST(CVT_COLOR_DVPP, RGB2GRAY) { cvtColorTestDVPP(COLOR_RGB2GRAY, 3, 1, 10.0f); }

TEST(CVT_COLOR, GRAY2BGR) { cvtColorTest(COLOR_GRAY2BGR, 1); }
// TEST(CVT_COLOR_DVPP, GRAY2BGR) { cvtColorTestDVPP(COLOR_GRAY2BGR, 1); }

TEST(CVT_COLOR, GRAY2BGRA) { cvtColorTest(COLOR_GRAY2BGRA, 1, 4); }
// TEST(CVT_COLOR_DVPP, GRAY2BGRA) { cvtColorTestDVPP(COLOR_GRAY2BGRA, 1, 4); }

TEST(CVT_COLOR, BGRA2GRAY) { cvtColorTest(COLOR_BGRA2GRAY, 4, 1, 10.0f); }
TEST(CVT_COLOR_DVPP, RGBA2GRAY) { cvtColorTestDVPP(COLOR_RGBA2GRAY, 4, 1, 10.0f); }
...
// TEST(CVT_COLOR_DVPP, BGR2YCrCb) { cvtColorTestDVPP(COLOR_BGR2YCrCb, 3, 3, 10.0f); }
TEST(CVT_COLOR, RGB2YCrCb) { cvtColorTest(COLOR_RGB2YCrCb, 3, 3, 10.0f); }
// TEST(CVT_COLOR_DVPP, RGB2YCrCb) { cvtColorTestDVPP(COLOR_RGB2YCrCb, 3, 3, 10.0f); }
...
TEST(CVT_COLOR_DVPP, BGR2YUV) { cvtColorTestDVPP(COLOR_BGR2YUV, 3, 3, 50.0f); }

...
PERF_TEST_P(NPU, CVT_COLOR, testing::Combine(TYPICAL_ASCEND_MAT_SIZES, CVT_COLORS_DVPP))
{
    Mat mat(GET_PARAM(0), CV_8UC3);
    Mat dst;
    declare.in(mat, WARMUP_RNG);
...
PERF_TEST_P(CPU, CVT_COLOR_DVPP, testing::Combine(TYPICAL_ASCEND_MAT_SIZES, CVT_COLORS_DVPP))
{
    Mat mat(GET_PARAM(0), CV_8UC3);
    Mat dst;
    declare.in(mat, WARMUP_RNG);

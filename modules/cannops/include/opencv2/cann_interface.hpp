...
@param stream AscendStream for the asynchronous version.
@note src must be one of the following types: float16, float32, double, int32,
int16, int8, int64, uint8, uint16, uint32, uint64.
...
images are supported. bicubic interpolation: Input images can be of different types, output images
must be float or uint8. pixel area interpolation: Input images can be of different types but output
images are always float.
...
/** @brief batch process for cropResizeMakeBorder.
@param batchSize  the number of images in a batch.
@note  The input images must be uint8, and only GRAY and BGR images are supported.

@sa cv::cann::cropResizeMakeBorder
*/
